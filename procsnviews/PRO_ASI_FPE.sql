...
CREATE DEFINER=`root`@`localhost` PROCEDURE `PRO_ASI_FPE`()
BEGIN
	...
	DECLARE CUR_2 CURSOR FOR
		...
		WHERE
			s.Status != 'A'
			AND s.STO_STATUS_ASI_FPE IS NULL
			...
			asignacion_lotes_loop: LOOP
				...
				-- lotes a los PosPedido completos de un DocEntrega incompleto.
					...
					UPDATE
						Stock
					SET
						STO_STATUS_ASI_FPE = 'A',
						STO_DOCENTREGA_ASI_FPE = var_DocEntrega,
						STO_POSPEDIDO_ASI_FPE = var_PosPedido
					WHERE
